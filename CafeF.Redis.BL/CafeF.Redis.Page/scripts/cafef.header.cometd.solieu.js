...
{
/*Updated 14/06/2011*/
document.location='/ceo/'+selected.data.c+'/'+this.UnicodeToKoDauAndGach(selected.data.o)+'.chn';
}
else
...
{
document.location='http://cafef.vn/Lich-su-giao-dich-Symbol-4/trang-1-'+selected.data.i+'.chn';
}
}
else
{
...
<!--$input.trigger("result", [selected.data, selected.value]);-->
document.location = autocomplete_GetCompanyInfoLink(selected.data.c); // "http://cafef.vn/Thi-truong-niem-yet/Thong-tin-cong-ty/"+selected.data.c+".chn";
//document.location = autocomplete_GetCeoInfoLink(selected.data.c);
return false;
}
/* tn chung khoan */
...
var __strTipLayer='<div id=\"cf_TipLayer\" style=\"display:none;position:absolute;z-index:100000;top:-100;\">' ;
...
this.SearchType = 0; // 0:Công ty; 1:Tin tức; 2:Ceo
...
output+= '<input class="s-input" style="display: none;" onkeydown="return CafeF_BoxSearch_OnEnter(event, 2);" value="Gõ họ tên ceo..." onblur="if (this.value == \'\') { this.value=\'Gõ họ tên ceo...\'; }" onfocus="this.value = \'\'; '+this.instance_name+'.InitAutoComplete();" type="text" id="CafeF_SearchKeyword_Ceo" autocomplete="off" class="class_text_autocomplete ac_input" />';
...
output+= ' <input type="radio" id="CafeF_BoxSearch_Type_Ceo" name="CafeF_BoxSearch_Type" onclick="'+this.instance_name+'.ChangeSearchType(2);" /> <label for="CafeF_BoxSearch_Type_Ceo">Ceo</label>';
output+= '</div></div>';
...
else if(searchType == '2')
{
    ...
    //alert(keyword);
    //if (keyword == 'Gõ họ tên ceo...') keyword = '';
...
jQuery('#CafeF_SearchKeyword_Ceo').autocomplete(ceooc, {
minChars: 1,
delay: 10,
width: 300,
matchContains: true,
...
{
    //keyword = document.getElementById('CafeF_SearchKeyword_Ceo').value;
    //var a = '/ceo/'+keyword.split('-')[0]+'/'+keyword.split('-')[0]+'.chn';
    //alert(a);
    /*Updated 14/06/2011*/
    //keyword = document.getElementById('CafeF_SearchKeyword_Ceo').value;
    //if (keyword == 'Gõ họ tên ceo...') keyword = '';
    //window.location = '/ceo/'+keyword.split('-')[0]+'/'+keyword.split('-')[0]+'.chn';
}
else
{
keyword = document.getElementById('CafeF_SearchKeyword_Company').value;
...
function LoadOverallHeader(){
    $.ajax({
        type: 'GET',
        url: '/Ajax/OverallHeader.aspx?ut=' + (new Date()).getTime(),
        data: 'r=1',
        success: function(data) {
...
function LoadHomepageBoxes(){
    $.ajax({
        type: 'GET',
        url: '/Ajax/HomepageBox.aspx?ut=' + (new Date()).getTime(),
        data: 'r=1',
        success: function(data) {
            var content = data.toString(); 
...
function loadboxhanghoa(tab) {
        ...
        if (html!=null && html != '') {
            ...
            $.ajax({
                type: "GET",
                url: "/frame_boxhanghoa.aspx",
                data: "tab=" + tab,
                success: function(msg) {
                    $('#boxhanghoa-content').html(msg);
                    $('#boxhanghoa-loaded').html($('#boxhanghoa-loaded').html() + "<div id='boxhanghoa-loadedtab" + tab + "'>" + msg + "</div>");
                }
            });
        }
    }