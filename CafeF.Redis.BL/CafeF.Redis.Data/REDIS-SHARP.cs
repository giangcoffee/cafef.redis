...
using System.Configuration;
    ...
    public class Redis : IDisposable
    {
        ...
        /// </summary>
        public Redis()
            : this(ConfigurationManager.AppSettings["ServerRedis"],int.Parse( ConfigurationManager.AppSettings["Port"]))//"192.168.44.23", 6379)
        {
        }
        ...
        public string GetString(string key)
        {
            if (key == null)
                throw new ArgumentNullException("key");
            return Get(key) != null?Encoding.UTF8.GetString(Get(key)):"";
        }
        ...
        public List<string> GetSortedSetString(string key, int start, int end)
        {
            if (key == null)
                throw new ArgumentNullException("key");

            byte[][] items = GetSortedSet(key, start, end);
            List<string> result = new List<string>();
        ...
        /// <summary>
        /// Return a range of elements from the SortedSet at key
        /// </summary>
        /// <param name="key">The key.</param>
        /// <param name="start">The start.</param>
        /// <param name="end">The end.</param>
        /// <returns></returns>
        public byte[][] GetSortedSet(string key, int start, int end)
        {
            if (key == null)
                throw new ArgumentNullException("key");

            if (!SendDataCommand(null, "ZRANGE {0} {1} {2}\r\n", key, start, end))
                throw new Exception("Unable to connect");
