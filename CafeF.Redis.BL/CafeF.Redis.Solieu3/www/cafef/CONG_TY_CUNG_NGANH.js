function CafeF_CongtyCungNganh(instanceName)
{
...
//        return query.toUpperCase();
    ...
    this.InitScript = function()
    {
        ...
        document.getElementById('CafeF_CongtyCungNganh').innerHTML = '<div class="cf_WCBox"><div class="cf_BoxHeader"><div></div></div><div class="cf_BoxContent" style="padding: 5px 10px 5px 10px;"><table class="Cafef_RelatedCompany" cellpadding="0" cellspacing="0"><tr><td style="font-weight: bold; font-size: 15px; text-align:left;">CÔNG TY CÙNG NGÀNH</td></tr><tr><td style="font-weight: bold; font-size: 13px;" id="CafeF_SameIndustry_Name"></td></tr><tr><td id="CafeF_SameIndustry_Dulieu" class="Dulieu"></td></tr><tr><td id="CafeF_SameIndustry_Phantrang" class="Phantrang"></td></tr><tr><td id="CafeF_SameIndustry_GhiChu" class="Ghichu"></td></tr></table></div><div class="cf_BoxFooter"><div></div></div></div>';
    }
    
    this.LoadData = function(pageIndex)
    {
        this.page_index = pageIndex;
        this.CreateScriptObject(this.host + '/ProxyHandler.ashx?RequestName=SameIndustry&CallBack=' + this.instance_name + '.OnLoaded&RequestType=json&Symbol=' + this.Symbol + '&PageSize=' + this.page_size + '&PageIndex=' + this.page_index);
    }
    
    this.OnLoaded = function(data, methodName)
    {
        var json = eval(data);
        var output = '<table cellpadding="3" cellspacing="0"><tr><th style="border-right: solid 1px #fff;text-align:left;">Mã CK</th><th style="border-right: solid 1px #fff;text-align:right;">Giá</th><th style="border-right: solid 1px #fff;text-align:right;">Thay đổi</th><th style="width:110px;">Vốn hóa thị trường<br />(Tỷ đồng)</th></tr>';
        var isAlternation = false;
        for (var i = 0; i < json.FinanceStatements.length; i++)
        {
            output += '<tr' + (isAlternation ? ' class="Alternation"' : '') + '><td class="Symbol"><a href="' + CafeF_JSLibrary.GetCompanyInfoLink(json.FinanceStatements[i].Symbol) + '">' + json.FinanceStatements[i].Symbol + '</a></td><td>' + this.FormatNumber(json.FinanceStatements[i].Price, true) + '</td><td>' + this.FormatChangeValue(json.FinanceStatements[i].Change, json.FinanceStatements[i].Percent) + '</td><td>' + this.FormatNumber(json.FinanceStatements[i].Capital, true) + '</td></tr>';
            isAlternation = !isAlternation;
        }
        output += '</table>';
        
        document.getElementById('CafeF_SameIndustry_Dulieu').innerHTML = output;
        document.getElementById('CafeF_SameIndustry_GhiChu').innerHTML = 'Trang ' + json.PageIndex + '/' + json.PageCount + ' (Tổng số: ' + json.RecordCount + ' công ty)';
        ...
        document.getElementById('CafeF_SameIndustry_Phantrang').innerHTML = page;
        if (json.OtherData) document.getElementById('CafeF_SameIndustry_Name').innerHTML = json.OtherData;
...
//document.write('<div id="CafeF_CongtyCungNganh"></div>');
var container = document.getElementById('CafeF_CongtyCungNganh');
if (container)
{
    var cafef_cong_ty_cung_nganh = new CafeF_CongtyCungNganh('cafef_cong_ty_cung_nganh');
    cafef_cong_ty_cung_nganh.InitScript();
    
    if (container.delay)
    {
        setTimeout('cafef_cong_ty_cung_nganh.LoadData(1)', parseInt(container.delay));
    }
    else
    {
        cafef_cong_ty_cung_nganh.LoadData(1);
    }
}