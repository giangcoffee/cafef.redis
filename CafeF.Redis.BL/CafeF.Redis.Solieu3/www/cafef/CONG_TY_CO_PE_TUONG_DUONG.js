function CafeF_CongtyCoPETuongduong(instanceName)
{
...
//        return query.toUpperCase();
    ...
    this.InitScript = function()
    {
        ...
        document.getElementById('CafeF_CongtyCoPETuongduong').innerHTML = '<div class="cf_WCBox"><div class="cf_BoxHeader"><div></div></div><div class="cf_BoxContent" style="padding: 5px 10px 5px 10px;"><table class="Cafef_RelatedCompany" cellpadding="0" cellspacing="0"><tr><td style="font-weight: bold; font-size: 15px; text-align:left;">CÔNG TY CÓ P/E TƯƠNG ĐƯƠNG</td></tr><tr><td id="CafeF_EquivalentPE_Dulieu" class="Dulieu"></td></tr><tr><td id="CafeF_EquivalentPE_Phantrang" class="Phantrang"></td></tr><tr><td id="CafeF_EquivalentPE_Note" class="Note"></td></tr><tr><td id="CafeF_EquivalentPE_GhiChu" class="Ghichu"></td></tr></table></div><div class="cf_BoxFooter"><div></div></div></div>';;
    }
    
    this.LoadData = function(pageIndex)
    {
        this.page_index = pageIndex;
        this.CreateScriptObject(this.host + '/ProxyHandler.ashx?RequestName=EquivalentPE&CallBack=' + this.instance_name + '.OnLoaded&RequestType=json&Symbol=' + this.Symbol + '&PageSize=' + this.page_size + '&PageIndex=' + this.page_index);
    }
    
    this.OnLoaded = function(data, methodName)
    {
        var json = eval(data);
        var output = '<table cellpadding="3" cellspacing="0"><tr><th style="border-right: solid 1px #fff;text-align:left;">Mã CK</th><th style="border-right: solid 1px #fff;text-align:right;">PE</th><th style="border-right: solid 1px #fff;text-align:right;">Giá</th><th style="text-align:right;">Thay đổi</th></tr>';
        var isAlternation = false;
        for (var i = 0; i < json.FinanceStatements.length; i++)
        {
            output += '<tr' + (isAlternation ? ' class="Alternation"' : '') + '><td class="Symbol"><a href="' + CafeF_JSLibrary.GetCompanyInfoLink(json.FinanceStatements[i].Symbol) + '">' + json.FinanceStatements[i].Symbol + '</a></td><td>' + (json.FinanceStatements[i].Price > 0 && json.FinanceStatements[i].EPS > 0 ? this.FormatNumber(json.FinanceStatements[i].PE, true) : 'N/A') + '</td><td>' + this.FormatNumber(json.FinanceStatements[i].Price, true) + '</td><td>' + this.FormatChangeValue(json.FinanceStatements[i].Change, json.FinanceStatements[i].Percent) + '</td></tr>';
            isAlternation = !isAlternation;
        }
        output += '</table>';
        
        document.getElementById('CafeF_EquivalentPE_Dulieu').innerHTML = output;
        document.getElementById('CafeF_EquivalentPE_Note').innerHTML = 'Danh sách các công ty có P/E tương đương trên sàn ' + (json.OtherData == 1 ? 'HoSE' : 'HaSTC') + '<br>(P/E +/- 1)';
        document.getElementById('CafeF_EquivalentPE_GhiChu').innerHTML = 'Trang ' + json.PageIndex + '/' + json.PageCount + ' (Tổng số: ' + json.RecordCount + ' công ty)';
        ...
        document.getElementById('CafeF_EquivalentPE_Phantrang').innerHTML = page;
        //document.getElementById('CafeF_EquivalentPE_Name').innerHTML = json.OtherData;
...
//document.write('<div id="CafeF_CongtyCoPETuongduong"></div>');
var container = document.getElementById('CafeF_CongtyCoPETuongduong');
if (container)
{
    var cafef_cong_ty_co_pe_tuong_duong = new CafeF_CongtyCoPETuongduong('cafef_cong_ty_co_pe_tuong_duong');
    cafef_cong_ty_co_pe_tuong_duong.InitScript();
 
    if (container.delay)
    {
        setTimeout('cafef_cong_ty_co_pe_tuong_duong.LoadData(1)', parseInt(container.delay));
    }
    else
    {
        cafef_cong_ty_co_pe_tuong_duong.LoadData(1);
    }
}