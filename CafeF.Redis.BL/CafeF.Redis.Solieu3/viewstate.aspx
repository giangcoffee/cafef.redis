        ...
        if (!string.IsNullOrEmpty(cacheName))
        {
            using (DataTable dt = GetCache(cacheName))
            {
                if (dt != null)
                {
                    ...
                    ltrThongbao.Text = GetCacheAsString(cacheName + "_LastUpdate");
                }
                else
                {
                    string data = GetCacheAsString(cacheName);
                    if (!string.IsNullOrEmpty(data))
                    {
                        data += "<br /><br />" + GetCacheAsString(cacheName + "_LastUpdate");
                        ltrThongbao.Text = data;
                    }
                    else
                    {
                        ltrThongbao.Text = "Khong co du lieu trong " + cacheName;
                    }
                }
            }
        }
